...
L2 = 0.0005 # penalty of L2 regularization
scale = 10000.0 # rescale output

# load data
with open('./sample_data50.pkl', 'rb') as f:
    data = pickle.load(f)
...
# print (train_data.shape, train_label.shape, np.argwhere(np.isnan(train_data)), np.argwhere(np.isnan(train_label)))
print ('compile model')
model = build_model()
